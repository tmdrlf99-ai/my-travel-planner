// MY TRAVEL PLANNER - password protected Vercel sync proxy
const {isAuthenticated,envReady}=require("./_site-auth");

const SUPABASE_URL =
  process.env.SUPABASE_URL ||
  "https://cqlcdexexsujklpwnyvy.supabase.co";
const SUPABASE_KEY =
  process.env.SUPABASE_SECRET_KEY ||
  process.env.SUPABASE_PUBLISHABLE_KEY ||
  "sb_publishable_7RaNiiyEa6XuDZzAgyO5Jg_uDI-l5hj";
...
async function sbFetch(table,method="GET",body=null,id=null){
  ...
  const query=method==="GET"?READ_QUERY[table]:(id!=null?`id=eq.${encodeURIComponent(id)}`:"");
...
module.exports=async function handler(req,res){
  ...
  if(!envReady()){
    return res.status(503).json({error:"사이트 비밀번호 환경변수 설정이 필요합니다."});
  }
  if(!isAuthenticated(req)){
    return res.status(401).json({error:"사이트 잠금 해제가 필요합니다.",code:"AUTH_REQUIRED"});
  }
